...
export default function FaqItem({ item, index }) {
    ...
    return (
        <div className='relative z-2 mb-16'>
            ...
            <div
                className={clsx(
                    'overflow-hidden transition-all duration-500 ease-in-out px-7',
                    active ? 'max-h-40 opacity-100 py-3.5' : 'max-h-0 opacity-0 py-0'
                )}
            >
                {activeId === item.id && (
            ...
            <div 
                className={clsx(
                    "g5 -bottom-7 -top-7 left-0 right-0 opacity-0 rounded-3xl -z-1 transition-opacity duration-500 absolute",
                    active && 'opacity-100'
                )}
            >
